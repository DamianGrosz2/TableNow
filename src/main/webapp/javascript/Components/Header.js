import '../../css/main.css';
import React from 'react';

function getInfos(props) {
    // This is simply a small component with three icons (Fork, Calendar, Time) that are used to show the user
    // information about the name of the restaurant, the date and the time of the reservation.
    return (
        <div className='header2'>
            <img src={icon2} alt="Not Found" className="IconImg2"/>
            <p className='headerInfo'>{props.name}</p>
            <img src={icon1} alt="Not Found" className="IconImg2"/>
            <p className='headerInfo'>{props.date}</p>
            <img src={icon3} alt="Not Found" className="IconImg2"/>
            <p className='headerInfo'>{props.time}</p>
        </div>
    );
}

function Header(props) {

    return (

        <div className="header1">
            <h1 className='header3'>{props.componentName}</h1>
            {props.displayInfos && getInfos(props)}
        </div>
    );
}

const icon1 = "data:image/png;base64,"
const icon2 = "data:image/png;base64,"
const icon3 = "data:image/png;base64,"

export default Header;